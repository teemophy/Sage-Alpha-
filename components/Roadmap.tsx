...
const Roadmap: React.FC = () => {
  return (
    <section id="journey" className="py-12 relative overflow-hidden bg-white/[0.01]">
      <div className="container mx-auto px-6">
        <div className="max-w-4xl mx-auto mb-10">
          <div className="text-[10px] font-mono text-gold mb-3 uppercase tracking-[0.4em] font-bold">Strategic Evolution</div>
          <h2 className="font-space text-4xl md:text-6xl font-bold mb-4 text-white">Roadmap</h2>
          <p className="text-slate-400 text-base md:text-lg font-light leading-relaxed">A disciplined multi-stage progression from validation to institutional scale.</p>
        </div>
            ...
            {PHASES.map((phase, index) => (
              <div key={phase.id} className={`relative flex flex-col md:flex-row items-center ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className="w-full md:w-[45%]">
                  <div className={`glass p-6 rounded-2xl border-l-2 ${phase.status === 'current' ? 'border-gold glow-gold' : 'border-white/5 opacity-50'}`}>
                    <div className="flex justify-between items-start mb-4">
                      <h3 className="font-space text-2xl font-bold text-white leading-tight">{phase.title}</h3>
                      {phase.status === 'current' && (
                        <span className="text-[8px] font-bold text-gold tracking-widest uppercase bg-gold/10 px-2.5 py-1 rounded">Current</span>
                      )}
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-slate-300 text-xs leading-relaxed">{phase.objective}</p>
                      </div>

                      {phase.trigger && (
                        <div className="space-y-1">
                          <div className="text-[9px] font-bold text-gold uppercase tracking-widest">Activation Trigger</div>
                          <p className="text-slate-300 text-xs italic">{phase.trigger}</p>
                        </div>
                      )}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {phase.participants.map(p => (
                            <span key={p} className="text-[9px] bg-white/5 border border-white/10 px-2 py-0.5 rounded text-slate-400 font-medium">{p}</span>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-6 mt-6 pt-6 border-t border-white/5">
                        <div className="space-y-0.5">
                          ...
                          <div className="text-[10px] text-white font-medium">{phase.structure}</div>
                        </div>
                        <div className="space-y-0.5">
                          <div className="text-[8px] font-bold text-slate-600 uppercase tracking-widest">Capacity</div>
                          <div className="text-[10px] text-white font-medium">{phase.capacity}</div>
                        </div>
                      </div>
                    </div>
                  </div>
